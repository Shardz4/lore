  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-50 font-sans">
      ...
      <aside className="w-64 border-r border-indigo-500/20 bg-slate-950/80 flex flex-col pt-8">
        ...
        <div className="p-4 mt-auto border-t border-indigo-500/20 flex flex-col gap-2">
          <div className="px-4 py-2 bg-slate-900/50 rounded-lg flex items-center gap-3 mb-2 border border-slate-800">
             <div className="w-8 h-8 bg-indigo-500/20 rounded-full flex items-center justify-center text-indigo-400 font-bold uppercase">
               {user?.email?.charAt(0) || "U"}
             </div>
             <div className="flex flex-col overflow-hidden">
               <span className="text-sm font-semibold truncate">{user?.displayName || "Operator"}</span>
               <button onClick={logout} className="text-xs text-slate-500 hover:text-red-400 text-left transition-colors flex items-center gap-1">
